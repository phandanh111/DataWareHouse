...
CREATE DATABASE DataWarehouse_MarketingAnalyticsStg;
...
USE DataWarehouse_MarketingAnalyticsStg;
...
CREATE TABLE [MarketingAnalyticStg] (
    [User_ID] int,
    [Date_ID] date,
    [Recency] int,
    [Response] int,
...
CREATE TABLE Fact_MarketingAnalytic (
	...
	[Total_Campaigns_Accepted] int,
    [Total_Spent] float,
	constraint PK_Fact primary key(CustomerKey, DateKey),
	constraint FK_CustomerKey FOREIGN KEY (CustomerKey) REFERENCES DimCustomer(CustomerKey), 
	constraint FK_DateKey FOREIGN KEY (DateKey) REFERENCES DimDate(DateKey), 
)
